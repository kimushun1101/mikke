...
Releases から自環境向けバイナリを取得し、`SHA256SUMS` を検証して `~/.local/bin` に配置する。配置先が PATH に無ければ、`$SHELL` に応じた追記先 (zsh は `~/.zshrc`、bash は `~/.bashrc`、macOS の bash は `~/.bash_profile`、fish は `fish_add_path` の実行) と追記する 1 行を案内する (設定ファイルは自動では変更しない)。追記前でもフルパス `~/.local/bin/mikke` でそのまま使える。既定で semantic 検索入りの full 版が入る。BM25 のみの slim 版は `| sh -s -- --slim`。Linux では glibc バージョン非依存の `-musl` (完全静的リンク) が選ばれるため、distro の glibc は気にしなくてよい。
...
以前の既定 `%LOCALAPPDATA%\Programs\mikke` に入れていた場合も、再実行すると新しい場所に入る。旧場所の `mikke.exe` はそのまま残る。以前の案内どおり旧場所をユーザー PATH に追加していた場合は、新しい配置先は PATH の末尾に足されるため旧 `mikke.exe` が先に見つかり、スクリプトが警告で知らせる (新版が効かない状態)。旧 `mikke.exe` は旧 installer の残骸なので削除し、ユーザー PATH の旧エントリも外す (スクリプトは案内するだけで、上書きや PATH の並べ替えはしない)。旧場所が PATH に無ければ `note:` で知らせるだけで、使われないので削除してよい。
...
`install.sh` / `install.ps1` は `~/.local/bin`、`cargo install` は `~/.cargo/bin` と配置先が違う。両方に入れると PATH の先頭側だけが使われる (rustup の shell 設定は `~/.cargo/bin` を先頭に足す)。スクリプトは配置後に `installed:` (今回配置したファイル) と、PATH 上で `mikke` が見つかれば `active:` (`mikke` と打つと実際に起動するもの) を分けて表示し、両者が別の実体なら warning (install.sh は `warning:` 行、install.ps1 は PowerShell の警告ストリーム) でその場所と `--version` の出力を示す (両 OS 共通)。上書きや PATH の書き換えはせず、所有元に応じた対処を案内する: `~/.cargo/bin` (`$CARGO_HOME/bin`) の `mikke` なら cargo 管理なので、cargo 版を使い続けるなら `cargo install` で更新し、スクリプト版へ移るなら先に `cargo uninstall mikke` で消す (スクリプトで `~/.cargo/bin` を上書きすると cargo の管理情報と食い違うため)。それ以外は所有元 (パッケージマネージャ等) で更新・削除するか、PATH の順序を見直す。実際に動く方は `command -v mikke` (PowerShell は `Get-Command mikke`) と `mikke --version` (full 版は `+semantic` 表記) で確認する。
